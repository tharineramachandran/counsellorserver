    ...
    function (err, user, info) {
      ...
      req.logIn(user, function (err) {
        ...
        if (user.hasOwnProperty("rows")) {
          if (
            user.rows[0].IS_COUNSELLOR != 1 ||
            user.rows[0].IS_COUNSELLOR != 2
          ) {
    ...
    if (reqUser.user) {
      if (reqUser.user.hasOwnProperty("rows")) {
        ...
        const user = await pool.query(
          'SELECT "TX_PICTURE",  "TX_VERIFICATION_STATUS","TX_IS_COMPLETED", "IS_COUNSELLOR" , "ID_USER_UUID" , "TX_USER_EMAIL" , "TX_USER_NAME" ,"ID_USER_UUID"   FROM "T_USER" WHERE "ID_USER_UUID" = $1',
          [reqUser.user.rows[0].ID_USER_UUID]
        );
