    ...
    try {
        ...
        var chat = []; 
            ...
            let newUser = await pool.query(
                'INSERT INTO "CT_USER_CHAT" ( "ct_user1", "ct_user2", "ct_subject", "ct_latest", "ct_unread", "ct_catagory" ) VALUES ($1,$2 ,$3,$4,$5,$6) RETURNING *',
                [data.user1, data.user2,   data.subject,  0,  1,data.catagory          ]
              );
              chat= newUser.rows[0];
          
              let newMessage  = await pool.query(
                'INSERT INTO "CT_USER_MESSAGES" ("ct_message","ct_sender","ct_receiver","ct_chatid","ct_order" ) VALUES ($1,$2,$3,$4,$5 ) RETURNING *',
                [  data.message,      data.user1, data.user2,chat.id, 1]
              );
...
router.post("/createMessages", authorization, async (req, res) => {
    try {
      ...
      console.log(order);
        let newMessage  = await pool.query(
          'INSERT INTO "CT_USER_MESSAGES" ("ct_message","ct_sender","ct_receiver","ct_chatid","ct_order" ) VALUES ($1,$2,$3,$4,$5 ) RETURNING *',
          [  data.message,      data.userID, data.receiverID, data.chatID, order]
        );
        const messagde  =   await pool.query('UPDATE "CT_USER_CHAT" SET "ct_unread" = $1 WHERE "id" = $2',
      [1, data.chatID]);
...
router.post("/read/:id", authorization, async (req, res) => {
  try {
      ...
      console.log(id);
      const message  =   await pool.query('UPDATE "CT_USER_CHAT" SET "ct_unread" = $1 WHERE "id" = $2',
      [0, id]);
      res.json( message );
