...
state:JSON.stringify({ counsellorID , requestID}),
      ...
      if (response == '1' && updatedRequest.rowCount > 0) {
        ...
        if (user.rowCount > 0) {
          ...
          var event = {
            ...
            'start': {
              'dateTime':  updatedRequest.rows[0].ct_session_start_time.replace("+","-"),
              'timeZone': 'Asia/Singapore',
            },
            'end': {
              'dateTime': updatedRequest.rows[0].ct_session_end_time.replace("+","-"),  
              'timeZone': 'Asia/Singapore',
            },
            'attendees': [
  ...
  if (id == 'callback') {
    try {
     ...
     await pool.query('DELETE  FROM "CT_COUNSELLOR_CAL_AUTH" WHERE "ct_counsellor_id" = $1',
      [state.counsellorID]);

      let newUser = await pool.query(
        'INSERT INTO "CT_COUNSELLOR_CAL_AUTH" ( "ct_accesstoken","ct_scope","ct_expirydate","ct_token_type","ct_refresh_token" ,"ct_counsellor_id") VALUES ($1, $2,$3,$4,$5  ,$6) RETURNING *',
        [tokens.access_token, tokens.scope, tokens.expiry_date, tokens.token_type, tokens.refresh_token, state.counsellorID]
      );
      const insertID = newUser.rows[0].id;
      
      res.redirect(Redirect.format({
        pathname: CLIENT_BASEURL_PAGE_URL + "/request/google/get",
        query: {
