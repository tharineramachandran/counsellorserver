...
const Edit_Fourth = ({ formData, setForm, navigation, step }) => {
  ...
  useEffect(() => {
    axios
      .get(baseURLAPI + "/Counsellor/availableDates/" + localStorage.userID, {
        headers: {
    ...
    if (step.id == "Counsellor_about") setIcon_name("circle");
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      if (!validateResults.results) {
        ...
        if (!validateResults.isNotSameDate) {
          resultsDisplay += "has the same time\r\n";
        }
  ...
  useEffect(() => {
    if (step.id == "Counsellor_availability") setIcon_name("circle");
    else setIcon_name("circle outline");
  }, []);
  ...
  const _handleSubmitClick = () => {
    ...
    if (done) {
      ...
      axios
        .post(
          baseURLAPI + "/Counsellor/UpdateTimeTable",
          { formData: formData, days: days, COUNSELLORID: localStorage.userID },
                          ...
                          <Grid.Column width={16}>
                            ...
                            <Form.Group widths="equal">
                              <Form.Field className="CustomForm">
                                ...
                                >
                                  <Icon name="send" className="appBanner" />{" "}
                                  Update Details &nbsp;&nbsp;
                                </Button>{" "}
                                &nbsp;&nbsp;&nbsp;
                              </Form.Field>
