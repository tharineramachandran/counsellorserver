...
class ViewNoti extends React.Component {
    ...
    setTable = () => {
        axios.get(baseURLAPI + '/notification/getNoti/' + localStorage.userID, {
            headers: {
                jwtToken: localStorage.jwtToken
            },
            data: {
        ...
        var panes =
            ...
            [  
             ...
             {    menuItem: "clear inbox",       render: () => <Tab.Pane>
                    
                            <Card style={{ width: '100%' }} >
                                <Card.Content>
                                    <Card.Header>
                                     No notifications for you </Card.Header>
                                </Card.Content>
                            </Card>
    ...
    markasread = () => {
        axios.post(baseURLAPI + '/notification/read/' + localStorage.userID, {
            userID: localStorage.userID
        }, {
            headers: {
                jwtToken: localStorage.jwtToken
    ...
    deleteall = () => {
        axios.post(baseURLAPI + '/notification/deleteNoti/' + localStorage.userID, {
            userID: localStorage.userID
        }, {
            headers: {
                jwtToken: localStorage.jwtToken
...
if (data.activeIndex == 1){
    this.deleteall();  console.log("success deleteall");
}
            ...
            <Grid style={{ height: '400px' }} columns='equal' divided>
                < Grid.Row >
                    <Grid.Column>
                        ...
                        <Tab menu={{ color: "teal", attached: false, tabular: false }} 
                        renderActiveOnly= {true} onTabChange={this.handleChange}
